...
if not TOKEN or not WEBHOOK_URL:
    raise ValueError("❌ Укажи переменные окружения: TOKEN и WEBHOOK_URL")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'enter_ip':
        ...
        ip, port, method = info.get('ip'), info.get('port'), info.get('method')
        ...
        await query.edit_message_text(f"✅ Успешно: {success}\n❌ Ошибок: {fail}")
        return ConversationHandler.END
    elif data == 'reset':
        user_data[user_id] = {}
