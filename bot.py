...
def start(message):
    ...
    # Предлагаем выбрать язык — азербайджанский или русский
    markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    ...
    # Инициализируем пользователя без языка, ждем выбора
    users[user_id] = {
        "sex": None,
        "interest": None,
        "partner": None,
...
def chat_handler(message):
    ...
    if user_id not in users or users[user_id].get("lang") is None:
        # Если пользователь еще не выбрал язык
        start(message)
        return
...
# Медиа обработчики для пересылки партнёру
